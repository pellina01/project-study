class feedback:
    ...
    def __init__(self, mqtt_url, sensor_function, corrective_function, lower_limit, device_name):
        ...
        self.mq_do = self.mqtt("do", mqtt_url)
        self.prev_status = False
        print("successful establishing connection with mqtt")

    def check(self):
        try:
            self.value = self.read_sensor_value()[1]
            self.mq_do.send(self.do_serializer())
            if self.lower_limit < self.value:
                self.feedback_is_on = False
            else:
    ...
    def do_serializer(self):
        return self.json.dumps({"status": "ok", "value": str(self.value)})
